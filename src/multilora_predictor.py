...
class MultiloraPredictor:
    ...
    def predict(self, image_path, mask_path):
        outputs = {}
        for target_object in self.models.keys():
            model = LoraSamInference(self.model_checkpoint, f"finetuned_weights/{target_object}/{self.weather}/lora_rank{self.rank}.safetensors", self.rank)
